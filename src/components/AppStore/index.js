...
class AppStore extends Component {
  state = {inputValue: '', activeTabId: tabsList[0].tabId}
  ...
  render() {
    const {inputValue, activeTabId} = this.state
    const filteredAppsList = appsList.filter(
      appItem =>
        appItem.category === activeTabId &&
        appItem.appName.toLowerCase().includes(inputValue.toLocaleLowerCase()),
    )
    return (
      <div className="bg-container">
        ...
        <ul className="appList-container">
          {filteredAppsList.map(appDetails => (
